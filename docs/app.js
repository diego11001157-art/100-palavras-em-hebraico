...
function keyboardHTML(inputId){const K=DATA.morph.keyboard||{letters:[],marks:[]};return`<div class="keyboard" data-keyboard="${esc(inputId)}"><h4>Teclado hebraico</h4><div class="keyrow">${K.letters.map(x=>`<button type="button" class="hkey" data-hkey="${esc(x)}">${esc(x)}</button>`).join('')}</div><div class="keyrow">${K.marks.map(x=>`<button type="button" class="hkey mark" data-hkey="${esc(x)}">${esc(x)}</button>`).join('')}</div><div class="keyrow"><button type="button" class="hkey control" data-hcontrol="back">⌫ apagar</button><button type="button" class="hkey control" data-hcontrol="clear">limpar</button><button type="button" class="hkey control" data-hkey="∅">∅ nenhum</button></div></div>`}
function bindKeyboard(box,inputId,onInput){const input=$(`#${inputId}`);if(!input)return;box.querySelectorAll('[data-hkey]').forEach(b=>b.onclick=()=>{const k=b.dataset.hkey;if(k==='∅')input.value='∅';else{if(input.value==='∅')input.value='';input.value+=k}input.dispatchEvent(new Event('input'))});box.querySelectorAll('[data-hcontrol]').forEach(b=>b.onclick=()=>{if(b.dataset.hcontrol==='clear')input.value='';else input.value=[...input.value].slice(0,-1).join('');input.dispatchEvent(new Event('input'))});if(onInput)input.addEventListener('input',onInput)}
...
function newBuildQuestion(){
  const type=$('#buildConj').value,mode=$('#buildMode').value,arr=DATA.morph[type];
  ...
  if(mode==='ending'){
    const body=item.body??item.form,expected=item.suffix||'∅';
    $('#buildGame').innerHTML=`<div class="quiz"><div class="qmeta">${title} Qal • paradigma forte שמר</div><div class="prompt" style="font-size:30px">${esc(item.reading)}</div><div class="session-note"><span>Alvo morfológico: <strong>${esc(item.pgn)}</strong></span><span>Complete somente a terminação</span></div><div class="build-stage"><div><div class="compose-box"><div class="smallnote">Forma em construção</div><div class="compose-line"><span class="compose-base hebrew" id="composePreview">${esc(body)}</span></div><label style="text-align:left;margin-top:8px">Terminação / aformativo</label><input id="buildEnding" class="hebrew" autocomplete="off" placeholder="toque numa terminação ou escreva"><div class="marker-grid">${suf.map(x=>`<button type="button" class="marker-btn" data-ending="${esc(x)}">${esc(x)}</button>`).join('')}</div></div></div>${keyboardHTML('buildEnding')}</div><button class="btn gold" id="checkBuild" style="margin-top:15px">Conferir</button><div class="feedback" id="buildFeedback"></div><div id="buildAnswer"></div></div>`;
    const box=$('#buildGame');const inp=$('#buildEnding');const update=()=>{$('#composePreview').textContent=body+(normMarker(inp.value)==='∅'?'':inp.value)};box.querySelectorAll('[data-ending]').forEach(b=>b.onclick=()=>{inp.value=b.dataset.ending;update()});bindKeyboard(box,'buildEnding',update);update();
  }else{
    $('#buildGame').innerHTML=`<div class="quiz"><div class="qmeta">${title} Qal • paradigma forte שמר</div><div class="prompt" style="font-size:30px">Alvo: <strong>${esc(item.pgn)}</strong></div><p class="smallnote">Reconheça as duas bordas da forma: preformativo e aformativo.</p><div class="form-row" style="text-align:left;margin-top:12px"><div><label>Prefixo / preformativo</label><input id="buildPrefix" class="hebrew" placeholder="∅ ou marcador"><div class="marker-grid">${pref.map(x=>`<button type="button" class="marker-btn" data-fill="prefix" data-marker="${esc(x)}">${esc(x)}</button>`).join('')}</div></div><div><label>Terminação / aformativo</label><input id="buildSuffix" class="hebrew" placeholder="∅ ou terminação"><div class="marker-grid">${suf.map(x=>`<button type="button" class="marker-btn" data-fill="suffix" data-marker="${esc(x)}">${esc(x)}</button>`).join('')}</div></div></div><div style="margin-top:12px">${keyboardHTML('buildSuffix')}</div><button class="btn gold" id="checkBuild" style="margin-top:15px">Conferir</button><div class="feedback" id="buildFeedback"></div><div id="buildAnswer"></div></div>`;
    const box=$('#buildGame');box.querySelectorAll('[data-fill]').forEach(b=>b.onclick=()=>{$(b.dataset.fill==='prefix'?'#buildPrefix':'#buildSuffix').value=b.dataset.marker});bindKeyboard(box,'buildSuffix');
  }
...
function normalizedParadigmForms(){const out=[];['perfect','imperfect','imperative'].forEach(category=>DATA.morph[category].forEach(item=>out.push({...item,category,categoryLabel:catLabel(category),stem:item.stem||'Qal',root:item.root||'שמר',gloss:item.gloss||'guardar',finite:true,sourceType:item.sourceType||'Kelley',source:item.source||'Kelley — paradigma forte'})));return out}
function allVerbalForms(){return[...normalizedParadigmForms(),...(DATA.morph.verbalForms||[])]}
